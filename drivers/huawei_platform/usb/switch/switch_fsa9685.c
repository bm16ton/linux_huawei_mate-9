...
static void rt8979_intb_work(struct work_struct *work)
{
	...
	muic_status1 = fsa9685_read_reg(RT8979_REG_MUIC_STATUS1);
	muic_status2 = fsa9685_read_reg(RT8979_REG_MUIC_STATUS2);
...
static ssize_t bootmode_store(struct device *dev,
                          struct device_attribute *attr, const char *buf, size_t size)
{
	int ret = 0;
	ret = fsa9685_write_reg(FSA9685_REG_MANUAL_SW_2, 0<<1);
...
static ssize_t switchuart_store(struct device *dev,
                          struct device_attribute *attr, const char *buf, size_t size)
{
	int ret = 0;
	int value = 0;
	...
	fsa9685_manual_sw(FSA9685_UART_ID_TO_IDBYPASS);

	ret &= (~FSA9685_MANUAL_SW);
    if ( ret < 0 ){
        ret = -ERR_FSA9685_REG_MANUAL_SW_1;
        hwlog_err("%s: write reg FSA9685 enable manual switch error!!! ret=%d\n", __func__, ret);
        return ret;
    }


    ret = fsa9685_write_reg(FSA9685_REG_MANUAL_SW_1, REG_VAL_FSA9685_UART_ID_TO_IDBYPASS);
    if ( ret < 0 ){
        ret = -ERR_FSA9685_REG_MANUAL_SW_1;
        hwlog_err("%s: write reg FSA9685_REG_MANUAL_SW_1 error!!! ret=%d\n", __func__, ret);
        return ret;
...
/*create a node for phone-off current drain test*/
    ...
    ret = device_create_file(&client->dev, &dev_attr_switchuart);
    if (ret < 0) {
        hwlog_err("%s: device_create_file error!!! ret=%d.\n", __func__, ret);
        ret = -ERR_SWITCH_USB_DEV_REGISTER;
        goto err_create_switchuart_failed;
    }

    ret = device_create_file(&client->dev, &dev_attr_bootmode);
    if (ret < 0) {
        hwlog_err("%s: device_create_file error!!! ret=%d.\n", __func__, ret);
        ret = -ERR_SWITCH_USB_DEV_REGISTER;
        goto err_create_bootmode_failed;
    }
...
err_create_fcp_mmi_failed:
	device_remove_file(&client->dev, &dev_attr_bootmode);
err_create_bootmode_failed:
    device_remove_file(&client->dev, &dev_attr_jigpin_ctrl);
err_create_jigpin_ctrl_failed:
	device_remove_file(&client->dev, &dev_attr_switchuart);
err_create_switchuart_failed:
