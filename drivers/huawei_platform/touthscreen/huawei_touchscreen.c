...
int put_one_cmd_thread(struct ts_cmd_node *cmd, int timeout)
{
	int error = -EIO;
	unsigned long flags;
	struct ts_cmd_queue *q;
	struct ts_cmd_sync *thread_sync = NULL;
	TS_LOG_INFO("%s Enter\n",__func__);
	if (!cmd) {
	...
	if (timeout) {
		...
		init_completion(&thread_sync->done);
		atomic_set(&thread_sync->timeout_flag, TS_NOT_TIMEOUT);
		cmd->thread_sync = thread_sync;
	} else {
		cmd->thread_sync = NULL;
	}
	error = put_one_cmd(cmd,NO_SYNC_TIMEOUT);
	TS_LOG_INFO("%s Put Command and wait, with timeout %d\n",__func__,timeout);
	if (timeout
	    && !(wait_for_completion_timeout(&thread_sync->done, abs(timeout) * HZ))) {
		atomic_set(&thread_sync->timeout_flag, TS_TIMEOUT);
		TS_LOG_ERR("wait for thread cmd respone timeout\n");
		error = -EBUSY;
		goto out;
	}
	TS_LOG_INFO("%s Wait End\n",__func__);
free_thread_sync:
	if (thread_sync){
		kfree(thread_sync);
	}
...
static ssize_t ts_oem_info_show(struct device *dev, struct device_attribute *attr, char *buf)
{
		...
		cmd->cmd_param.prv_params = (void *)info;
		if( g_ts_data.chip_data->is_put_cmd_thread ) {
			TS_LOG_INFO("%s:use put_one_cmd_thread func\n", __func__);
			error = put_one_cmd_thread(cmd, LONG_SYNC_TIMEOUT);
		} else {
			error = put_one_cmd(cmd, LONG_SYNC_TIMEOUT);
		}
		if (error) {
			   ...
			   const char *buf, size_t count)
{
	...
	cmd->cmd_param.prv_params = (void *)info;
	if( g_ts_data.chip_data->is_put_cmd_thread ) {
		TS_LOG_INFO("%s:  put_one_cmd_thread\n", __func__);
		error = put_one_cmd_thread(cmd, LONG_LONG_SYNC_TIMEOUT);
	}else{
			  ...
			  char *buf)
{
	...
	info =
	    ...
	    kzalloc(sizeof(struct ts_chip_info_param), GFP_KERNEL);
	if (!info) {
		TS_LOG_ERR("malloc failed\n");
		error = -ENOMEM;
		goto out;
				  ...
				  struct device_attribute *attr, char *buf)
{
	...
	if( g_ts_data.chip_data->is_put_cmd_thread ) {
		TS_LOG_INFO("%s:  put_one_cmd_thread\n", __func__);
		error = put_one_cmd_thread(cmd, SHORT_SYNC_TIMEOUT);
	}else{
		error = put_one_cmd(cmd, SHORT_SYNC_TIMEOUT);
	}
	if (error) {
...
static int ts_send_roi_cmd(enum ts_action_status read_write_type, int timeout)
{
	...
	if (g_ts_data.feature_info.roi_info.roi_supported) {
		...
		if( g_ts_data.chip_data->is_put_cmd_thread ) {
			TS_LOG_INFO("%s:  put_one_cmd_thread\n", __func__);
			error = put_one_cmd_thread(&cmd, timeout);
		}else{
				 ...
				 size_t count)
{
	...
	TS_LOG_INFO
	    ("parameter data is flag:%d, addr:%d, value:%s, num:%d, bit:%d\n",
...
static void rotate_rawdata_abcd2cbad(int row, int column, int *data_start)
{
...
static int rawdata_proc_show(struct seq_file *m, void *v)
{
	...
	if (g_ts_data.chip_data->rawdata_get_timeout){
		if (g_ts_data.chip_data->is_put_cmd_thread){
			error = put_one_cmd_thread(cmd, g_ts_data.chip_data->rawdata_get_timeout);
		}else{
	...
	if (g_ts_data.chip_data->rawdata_arrange_type == TS_RAWDATA_TRANS_ABCD2CBAD) {
		...
		row_size = info->buff[1];
		range_size = info->buff[0];
		rotate_rawdata_abcd2cbad(row_size, range_size,
					 info->buff + 2 + row_size * range_size);
		rotate_rawdata_abcd2cbad(row_size, range_size, info->buff + 2);
	}
				  ...
				  struct ts_cmd_node *out_cmd)
{
	...
	if( g_ts_data.chip_data->is_put_cmd_thread ) {
		g_ts_data.thread_sync = in_cmd->thread_sync;
	}
...
static int chip_detect(const char *chip_name)
{
	...
	for (index = 0; ptr != NULL && index < ARRAY_SIZE(g_ts_device_map);
		...
		if (!strcmp(chip_name, ptr->chip_name)) {
			...
			g_ts_data.chip_data->is_put_cmd_thread= 0;
...
void check_tp_calibration_info(void)
{
	...
	if (info->calibration_crc) {
#if defined (CONFIG_HUAWEI_DSM)
		if (!dsm_client_ocuppy(tp_dclient)) {
			TS_LOG_INFO("try to client record %d for calibration crc\n", DSM_TP_CALIBRATION_CRC_ERROR_NO);
			dsm_client_record(tp_dclient, "tp calibration crc is :%d\n", info->calibration_crc);
			dsm_client_notify(tp_dclient, DSM_TP_CALIBRATION_CRC_ERROR_NO);
		}
#endif
	}
out:
