...
static ssize_t new_device2_store(struct device *dev,
				struct device_attribute *attr,
				const char *buf, size_t count)
{
	struct spi_master *master = container_of(dev, struct spi_master, dev);
	struct ftdi_spi *priv = spi_master_get_devdata(master);
...
//	};
    ...
    pd->spi_info[3].mode = SPI_MODE_0;
    pd->spi_info[3].max_speed_hz = 30000000;
    
	rc = strscpy(pd->spi_info[3].modalias, str, sizeof(&pd->spi_info[3].modalias));
	if (rc < 0)
		goto free_req;

	str = strsep(&req, " ");
	rc = kstrtou16(str, 0, &pd->spi_info[3].chip_select);
	if (rc)
		goto free_req;
...
/*
	...
	sdev = spi_new_device(master, &pd->spi_info[3]);
	if (!sdev)
		goto free_req;
...
static DEVICE_ATTR_WO(new_device2);
...
static void remove_sysfs_attrs(struct usb_interface *interface)
{
			...
			device_remove_file(&interface->dev, &dev_attr_new_device2);
}

static void ftdi_spi_set_cs(struct spi_device *spi, bool enable)
{
	struct ftdi_spi *priv = spi_master_get_devdata(spi->master);
...
static int ftdi_spi_transfer_one(struct spi_master *ctlr,
				 struct spi_device *spi,
				 struct spi_transfer *xfer)
{
	struct ftdi_spi *priv = spi_master_get_devdata(ctlr);
	struct device *dev = &priv->pdev->dev;
	int ret = 0;
...
static int ftdi_spi_init_io(struct spi_master *master, unsigned int dev_idx)
{
	struct ftdi_spi *priv = spi_master_get_devdata(master);
	struct platform_device *pdev = priv->pdev;
	const struct mpsse_spi_platform_data *pd;
