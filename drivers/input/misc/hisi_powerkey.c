...
#include "../../video/fbdev/hisi/dss/backlight/lm36923.h"
...
/*
	bl_level=lm36923_blget();
	if (bl_level == 0) {
	ret = blquickset(oldblval);
	if (ret) {
	}
		} else if (bl_level > 0) {
		oldblval = bl_level;
...
*/
		...
		input_report_key(info->idev, KEY_F12, POWER_KEY_PRESS);
		input_sync(info->idev);
		input_report_key(info->idev, KEY_F12, POWER_KEY_RELEASE);
		input_sync(info->idev);
...
#endif
	__set_bit(KEY_F12, info->idev->keybit);
