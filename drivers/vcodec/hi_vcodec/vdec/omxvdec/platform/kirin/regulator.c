...
/*----------------------------------------
 ...
 ----------------------------------------*/
static HI_U64 VDEC_Enable_Iommu(struct device *dev)
{
    HI_S32 ret;
...
static HI_S32 VDEC_GetDtsConfigInfo(struct device *dev, VFMW_DTS_CONFIG_S *pDtsConfig)
{
    ...
    {
        OmxPrint(OMX_FATAL, "%s irq_of_parse_and_map MfdeSafeIrqNum failed!\n", __func__);
        return HI_FAILURE;
    }
    ...
    {
        OmxPrint(OMX_FATAL, "%s irq_of_parse_and_map ScdSafeIrqNum failed!\n", __func__);
        return HI_FAILURE;
    }
    ...
    {
        OmxPrint(OMX_FATAL, "%s irq_of_parse_and_map BpdSafeIrqNum failed!\n", __func__);
        return HI_FAILURE;
    }
    ...
    {
        OmxPrint(OMX_FATAL, "%s irq_of_parse_and_map SmmuSafeIrqNum failed!\n", __func__);
        return HI_FAILURE;
    }
    ...
    if (ret)
    {
        OmxPrint(OMX_FATAL, "%s of_address_to_resource crg failed! return %d\n", __func__, ret);
        return HI_FAILURE;
    }
