...
case $imgtype in
  ...
  U-Boot)
    name=`echo uboot.img`;                   echo "name = $name";
    uarch=`echo arm64`;
    os=`echo linux`;
    type=`echo kernel`;
    comp=`echo lz4`;                   echo "type = $uarch $os $type ($comp)";
    [ "$comp" = "uncompressed" ] && comp=none;
    addr=`echo 0x00008000`;                   echo "load_addr = $addr";
    ep=`echo 0x00078000`;                       echo "entry_point = $ep";
  ;;
  *)
    if [ -f *-second ]; then
